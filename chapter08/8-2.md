...
##### STEP 3
어서션을 추가하는 대신 문제 발생 시 로깅하는 방법도 있다. [(6)](https://github.com/wonder13662/refactoring-v2/blob/writing/chapter08/8-2.md#6) 시스템의 겉보기 동작이 달라지지 않는다는 확신이 서면 이자율을 가져오는 부분을 변경하고 [(8)](https://github.com/wonder13662/refactoring-v2/blob/writing/chapter08/8-2.md#8) `Account`에서 아지율을 직접 수정하던 코드를 완전히 제거한다.
``` diff
class Account {
- constructor(number, type, interestRate) {
